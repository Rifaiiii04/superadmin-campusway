...
try {
    echo "Starting final rumpun ilmu category fix...\n";
    ...
    // ILMU TERAPAN (20-25, 33-45, 49-58) - Semua sisanya masuk ke Ilmu Terapan
    $ilmuTerapan = [
        // Ilmu Terapan (20-25)
    ...
    // Update any remaining categories that don't match the 5 main categories
    $updated = DB::table('major_recommendations')
        ->whereIn('category', ['Ilmu Kesehatan', 'Ilmu Lingkungan', 'Ilmu Teknologi'])
        ->update(['category' => 'Ilmu Terapan']);
    echo "Updated remaining special categories to Ilmu Terapan: {$updated} records\n";
    ...
    foreach ($categories as $category) {
        $sample = DB::table('major_recommendations')
        ...
        foreach ($sample as $record) {
            echo "  - {$record->major_name}\n";
        }
    }
    
    echo "\nRumpun ilmu category fix completed successfully!\n";
